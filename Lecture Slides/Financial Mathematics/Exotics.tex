...
\frame{\frametitle{ Change of num{\'e}raire technique}
...
\frame{\frametitle{ Barrier Options}
...
\frame{\frametitle{ Down-and-out Call}

Consider a down-and-out call option with strike $K$ and barrier
$H$. The payoff is
$$
\begin{array}{ll}
...
\end{array}
$$
so by risk-neutral pricing the value of the option ${DOC}_{K,H}$
is
$$
...
\frame{\frametitle{ max and min of BM}
...
\frame{\frametitle{ Reflection Principle}
...
\frame{\frametitle{ Density of $(X(t),M^X(t))$}
...
\frame{\frametitle{ Valuation Formula}
...
$$
The option formula is, writing $\lambda := r -
\frac{1}{2}{\sigma}^2$,
$$
\begin{array}{lll}
...
\frame{\frametitle{Binary Options} We assume that the underlying
model is a  Black-Scholes model. Recall the payoffs for a European
call resp. put
$$
...
\frame{\frametitle{Binary Options}
...
<K\}}.
$$
{\it Asset-or-nothing options.}  Here the corresponding payoffs
are
$$
...
\frame{\frametitle{Paylater Options}
...
\frame{\frametitle{Compound Options}
...
\frame{\frametitle{Valuation formula}
Then
the price of a call on a call is
$$
\begin{array}{lll}
...
\frame{\frametitle{Asian Options}
...
of the underlying asset over the discrete set of monitoring dates
...
There are pricing approaches using transform analysis
(Laplace transforms!).

}

\frame{\frametitle{Geometric average option}
...
\frame{\frametitle{Pricing by simulation}
...
We can now compare the Monte-Carlo approach with the exact valuation
formula.
}
...
$$
denote
the sample standard deviation and let $z_\delta$ denote the
...
\prb(-z_{\delta/2}\leq(C-\hat{C}_n)\frac{\sqrt{n}}{S_c}\leq z_{\delta/2}) & = & 1-\delta\\*[12pt]
 ...
 & =& 1-\delta
\end{array}$$
For $\delta=0.05$ we have $z_{\delta/2}\approx 1.96$
for example.
}
\frame{\frametitle{Pricing by simulation -- Asian Options}
...
\frame{\frametitle{Indexed Options} Consider the following
...
\end{array}
$$
with all coefficients constant, and the volatility matrix $\s =
\left[\begin{array}{ll}
\s_{11} & \s_{12}\\
\s_{21} & \s_{22}
\end{array}
...
\frame{\frametitle{Indexed Options}
...
\frame{\frametitle{Options on the minimum/maximum of two stocks}
