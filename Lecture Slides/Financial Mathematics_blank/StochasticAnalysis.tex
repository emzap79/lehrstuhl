...
\begin{definition}\label{BM}
	...
	Brownian motion, $BM$, if
		...
		$X$ has {\it independent increments}: $X(t+u) - X(t)$ is 
		independent of $\s (X(s): s \leqt)$ for $u \geq 0$, 
...
We denote standard Brownian motion $BM(\setR)$ by
$W = (W(t))$ ($W$ for Wiener), though $B = (B(t))$ ($B$ for Brown)
is also common.  

Standard Brownian motion $BM({\setR}^d)$ in $d$
