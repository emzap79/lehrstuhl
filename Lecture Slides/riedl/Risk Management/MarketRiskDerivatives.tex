...
and lending
\item<3-> {\it Competitive markets:}  Market participants
act as price takers
\item<4-> {\it Rational agents:} Market participants prefer more to less.
\item<5-> \emph{We assume that arbitrage opportunities do not exist!}
\end{itemize}
}
...
\begin{itemize}
  \item An option is a financial instrument giving the holder the {\it right but
	not the obligation} to make a specified transaction at (or by)
	specified date(s) at a specified price ({\it the strike}).
  \item {\it Call} options give one the right to buy an asset (\emph{the
  	underlying}).
  \item {\it Put} options give one the right to sell an asset.
  \item {\it European} options give the holder the right
	to buy/sell on the specified date, the expiry date, on which the option expires
	or matures.
  \item {\it American} options give the holder the right to buy/sell at any time
	prior to or at expiry.
  \item \emph{Bermudan} options give the holder the right to buy/sell at one of a
	number of specified days.
\end{itemize}
}

\frame{\frametitle{European Call Options}
\begin{itemize}
  \item Consider, say, a European call option, with strike price $K$;
...
\item The payoff from the option is $$ S(T) - K \mbox{ if } S(T)
> K\A \mbox{ and }\A 0 \;\; \mbox{otherwise} $$ (more briefly
written as  $(S(T) - K)^+$).
\item We obtain the payoff diagram on the next slide. 
\end{itemize}
...
\item<3-> The settlement date is called {\it delivery date} and the specified
...
\begin{frame}[fragile]
\frametitle{Value of a Forward Contract}
\begin{block}{Note!}
...
\begin{itemize}
\item<1-> Futures can be defined as standardized forward contracts traded at
...
\item<4-> Thus credit risk is (virtually) eliminated, but there is exposure to
interest rate risk.
