...
Further properties of GH-distributions -- Computation of VaR/ES} 
	...
	following setting:
		...
		Compute value-at-risk (numerically) via $ VaR_{\alpha}(X)=F^{-1}_X(\alpha) $ 

		Expected shortfall is defined (for absolute continuous
		distributions) via $ ES_{\alpha}(X) =\frac{1}{1-\alpha}\int_{\alpha}^1VaR_u(X)du$. In case of
...
Some statistical results %\vspace{-1.5cm}
...
Coal-fired Power Plant
	We consider a portfolio that can be viewed roughly as the year production/consumption of a
...
% Unterschiedliches Tail-Verhalten macht auch die implizite Kalibrierung an Optionen mit unterschiedlichen Lieferperioden/Underlyings instabil.
	Same tail-behaviour in all components of $$X\stackrel{d}{=}\mu+W\gamma+\sqrt{W}AZ$$
...
Copulas
	We fit 5 copula models (Normal, t, Frank, Gumbel, Clayton) to the data.  
	
	Fitting is done via the IFM (Inference for margins) method, which means we fit the marginal models
	separately, employ the quantile transformation on each margin and then fit the copula on this transformed dataset.  
...
Copula fit -- Results
	t and normal copula provide the best fit,  because
		...
		they have more flexibility to fit the dependencebetween the margins of our models.
...
Coal-fired Power Plant -- Margins
\tiny
\begin{table}[ht]
                \begin{center}
                    \begin{tabular}{c|c|c|c|c|c|c|c|c}
